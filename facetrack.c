...
int main( int argc, char** argv )
{
	...
	cvSetCaptureProperty(capture, CV_CAP_PROP_FRAME_WIDTH, 640.0); // needed for stupid cameras like iSights
...
void camShifterLoop() {
	int i;
	for (i = 0; i < loops; i++) // only do i loops before breaking out and searching for new faces because camshift will eventually converge on something else
	{
		if (!camShifter()) break;
